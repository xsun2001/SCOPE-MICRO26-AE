...
class VectorUnit:
    def __init__(
        ...
        self.mufu_per_cycle = mufu_per_cycle
        self.total_mufu_ops_per_cycle = vector_width * vector_count * mufu_per_cycle
        self.flops_per_cycle = ceil(
            total_vector_flops_per_cycle / vector_width / vector_count
        )
        self.data_type = data_type

    def vector_cycles(self, flop_count):
        return flop_count / self.total_vector_flops_per_cycle

    def mufu_cycles(self, op_count):
        if op_count == 0:
            return 0
        if self.total_mufu_ops_per_cycle:
            return op_count / self.total_mufu_ops_per_cycle
        return op_count * self.flops_per_exp / self.total_vector_flops_per_cycle


vector_unit_dict = {
    "A100_fp16": VectorUnit(512, 2, 35, 32, 4, mufu_per_cycle=0.125),
...
class SystolicArray:
    def __init__(
        ...
        output_word_size,
        int8_multiplier=1,
        int8_input_word_size=None,
        int8_output_word_size=None,
    ):
        self.array_height = array_height
        self.array_width = array_width
        if not float(mac_per_cycle).is_integer():
            raise ValueError(
                f"mac_per_cycle must be an integer, got {mac_per_cycle}"
            )
        self.mac_per_cycle = int(mac_per_cycle)
        self.input_word_size = input_word_size
        self.output_word_size = output_word_size
        if not float(int8_multiplier).is_integer() or int8_multiplier <= 0:
            raise ValueError(
                f"int8_multiplier must be a positive integer, got {int8_multiplier}"
            )
        self.int8_multiplier = int(int8_multiplier)
        self.int8_input_word_size = (
            input_word_size if int8_input_word_size is None else int8_input_word_size
        )
        self.int8_output_word_size = (
            output_word_size if int8_output_word_size is None else int8_output_word_size
        )

    def mac_per_cycle_for(self, data_type: DataType):
        if data_type.name == "int8":
            return self.mac_per_cycle * self.int8_multiplier
        return self.mac_per_cycle

    def input_word_size_for(self, data_type: DataType):
        if data_type.name == "int8":
            return self.int8_input_word_size
        return self.input_word_size

    def output_word_size_for(self, data_type: DataType):
        if data_type.name == "int8":
            return self.int8_output_word_size
        return self.output_word_size
...
systolic_array_dict = {
    ...
    "MI210_fp16": SystolicArray(16, 16, 1, 2, 2),
...
class ComputeModule:
    def __init__(
        ...
        self.total_mufu_ops_per_cycle = (
            core.vector_unit.total_mufu_ops_per_cycle * core_count
        )
        self.total_mufu_ops = self.total_mufu_ops_per_cycle * clock_freq
        self.total_systolic_array_flops = self.total_systolic_array_flops_for(
            data_type_dict["fp16"]
        )
        self.overhead = overhead

    def vector_latency(self, flop_count):
        return flop_count / self.total_vector_flops

    def mufu_latency(self, op_count):
        if op_count == 0:
            return 0
        if self.total_mufu_ops:
            return op_count / self.total_mufu_ops
        return op_count * self.core.vector_unit.flops_per_exp / self.total_vector_flops

    def total_systolic_array_flops_for(self, data_type: DataType):
        return (
            self.core_count
            * self.core.systolic_array_count
            * self.core.systolic_array.mac_per_cycle_for(data_type)
            * 2
