...
class Softmax(Operator):
    ...
    def roofline_model(self, pcb_module: Device):
        ...
        self.flop_count = self.M * self.N * (vector_unit.flops_per_exp * 3 + 7)
        vector_latency = pcb_module.compute_module.vector_latency(self.M * self.N * 7)
        mufu_latency = pcb_module.compute_module.mufu_latency(self.M * self.N * 3)
        self.roofline_latency=max(self.io_count/min(pcb_module.io_module.bandwidth, pcb_module.compute_module.l2_bandwidth_per_cycle*pcb_module.compute_module.clock_freq), vector_latency + mufu_latency)
    ...
    class L1TileSimulator:
        def __init__(
            ...
            self.reduction_cycle_count = (
                pcb_module.compute_module.core.vector_unit.mufu_cycles(M * N)
                + pcb_module.compute_module.core.vector_unit.vector_cycles(M * N * 2)
                + M
                * N
                * data_type.word_size
                * 2
        ...
        def simulate_l1_tile_compute_cycle_count(
            ...
            # online softmax
            return ceil(
                pcb_module.compute_module.core.vector_unit.vector_cycles(M * N * 7)
                + pcb_module.compute_module.core.vector_unit.mufu_cycles(M * N * 3)
            )
