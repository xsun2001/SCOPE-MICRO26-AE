...
def _parse_numeric_or_inf(value):
...
def _word_size_from_data_type(data_type_name):
    return int(re.search(r"(\d+)", data_type_name).group(1)) // 8
...
def template_to_system(arch_specs):
    ...
    vector_count = core_specs.get("vector_count", sublane_count)
    systolic_array_count = core_specs.get("systolic_array_count", sublane_count)
    # vector unit
    ...
    vector_unit = VectorUnit(
        ...
        * vector_unit_specs["flop_per_cycle"],
        _word_size_from_data_type(vector_unit_specs["data_type"]),
        35,
        vector_unit_specs["vector_width"],
        vector_count,
        mufu_per_cycle=vector_unit_specs.get("mufu_per_cycle", 0),
    )
    # systolic array
    systolic_array_specs = core_specs["systolic_array"]
    systolic_input_data_type = systolic_array_specs.get(
        "input_data_type", systolic_array_specs["data_type"]
    )
    systolic_output_data_type = systolic_array_specs.get(
        "output_data_type", systolic_array_specs["data_type"]
    )
    systolic_int8_input_data_type = systolic_array_specs.get(
        "int8_input_data_type", "int8"
    )
    systolic_int8_output_data_type = systolic_array_specs.get(
        "int8_output_data_type", "int32"
    )
    systolic_array = SystolicArray(
        ...
        _word_size_from_data_type(systolic_input_data_type),
        _word_size_from_data_type(systolic_output_data_type),
        systolic_array_specs.get("int8_multiplier", 1),
        _word_size_from_data_type(systolic_int8_input_data_type),
        _word_size_from_data_type(systolic_int8_output_data_type),
    )
    # core
    core = Core(
        vector_unit,
    ...
    # compute module
    global_buffer_size = io_specs.get("global_buffer_byte")
    if global_buffer_size is None:
        global_buffer_size = io_specs["global_buffer_MB"] * 1024 * 1024
    compute_module = ComputeModule(
        core,
        compute_chiplet_specs["core_count"] * device_specs["compute_chiplet_count"],
        _parse_numeric_or_inf(device_specs["frequency_Hz"]),
        _parse_numeric_or_inf(global_buffer_size),
        _parse_numeric_or_inf(io_specs["global_buffer_bandwidth_per_cycle_byte"]),
        overhead_dict[device_specs.get("overhead_model", "A100")],
    )
    # io module
    io_bandwidth = io_specs.get("bandwidth_byte_per_second")
    if io_bandwidth is None:
        io_bandwidth = (
    ...
    io_module = IOModule(
        _parse_numeric_or_inf(io_bandwidth),
        _parse_numeric_or_inf(io_specs.get("latency_second", 1e-6)),
    )
    # memory module
    memory_specs = device_specs["memory"]
    memory_capacity = memory_specs.get("total_capacity_byte")
    if memory_capacity is None:
        memory_capacity = (
            _parse_numeric_or_inf(memory_specs["total_capacity_GB"]) * 1024 * 1024 * 1024
        )
    memory_module = MemoryModule(
        _parse_numeric_or_inf(memory_capacity)
    )
    # device
    device = Device(compute_module, io_module, memory_module)
    device.operator_latency_scales = arch_specs.get(
        "operator_latency_scales",
        device_specs.get("operator_latency_scales", {}),
    )
    # interconnect
    ...
    interconnect_module = InterConnectModule(
        ...
        _parse_numeric_or_inf(
            interconnect_specs.get("internal_link_bandwidth_per_direction_byte", float("inf"))
        ),
    )
