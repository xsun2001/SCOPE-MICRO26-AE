"""
PyTorch LLaMA attention with attention-exp replaced by a piecewise-linear NNLUT.
"""
...
from ...common import ScalarNnLut
...
def qeager_attention_forward(
    ...
    **kwargs,
    ...
    exp_elements_approx = module.exp_lut(valid_shifted_logits_reshaped)

    if args.quant_approx_activations:
        exp_elements_approx = module.approx_output_quantizer(exp_elements_approx)

    exp_approx[valid_positions_mask] = exp_elements_approx.squeeze(-1)
...
class LlamaAttention_NNLUT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_embeddings is None:
            raise ValueError("LlamaAttention_NNLUT requires precomputed position_embeddings.")
        ...
        if self.config._attn_implementation != "eager":
            raise ValueError("Only eager attention is supported for LlamaAttention_NNLUT")
