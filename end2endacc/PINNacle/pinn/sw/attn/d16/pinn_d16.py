...
class ScalarExpoNet_d16(nn.Module):
    def __init__(self, input_dim=1, output_dim=1):
        super(ScalarExpoNet_d16, self).__init__()
        self.fc1 = nn.Linear(input_dim, 16, bias=True)
        fc1_weight_raw = torch.empty(16, input_dim)
        nn.init.uniform_(fc1_weight_raw, a=-5, b=0)  
        self.fc1_weight_raw = nn.Parameter(fc1_weight_raw)

        fc1_bias_raw = torch.empty(16)
        nn.init.uniform_(fc1_bias_raw, a=-5, b=0)
        self.fc1_bias_raw = nn.Parameter(fc1_bias_raw)
        self.relu1 = nn.ReLU()

        fc2_weight_raw = torch.empty(output_dim, 16)
