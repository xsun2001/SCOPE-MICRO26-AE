...
Run through Slurm with `make reproduce`, or on an already allocated GPU with `make reproduce EXECUTOR=local WORKERS=1`. Use `make evidence` to recollect all 18 best-MSE comparisons from the bundled summaries and regenerate `runs/<run-id>/evidence/fig-20-shape-constraints/generated/figure20.{png,pdf}` from the bundled raw histories without a GPU. Evidence outputs are intentionally staged under the ignored run directory so review does not modify packaged files.
