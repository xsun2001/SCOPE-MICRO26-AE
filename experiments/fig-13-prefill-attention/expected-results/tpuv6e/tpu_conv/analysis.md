...
| TPUv3 INT8 | int8 | 32.00 | 306.152 | 143.634 | 126.387 | 1.136x |
| TPUv4 FP16 | fp16 | 64.00 | 362.271 | 129.593 | 96.685 | 1.340x |
| TPUv4 INT8 | int8 | 64.00 | 356.981 | 128.610 | 104.416 | 1.232x |
| TPUv5e FP16 | fp16 | 32.00 | 514.407 | 181.310 | 135.239 | 1.341x |
| TPUv5e INT8 | int8 | 32.00 | 374.148 | 90.123 | 73.362 | 1.228x |
| TPUv5p FP16 | fp16 | 32.00 | 175.651 | 77.876 | 58.131 | 1.340x |
| TPUv5p INT8 | int8 | 32.00 | 115.892 | 38.795 | 31.612 | 1.227x |
| TPUv6e FP16 | fp16 | 128.00 | 214.050 | 69.646 | 41.082 | 1.695x |
| TPUv6e INT8 | int8 | 128.00 | 180.116 | 58.051 | 30.278 | 1.917x |
...
| TPUv3 INT8 | 0.900 | 0.242 | 0.275 | 0.268x | 0.305x |
| TPUv4 FP16 | 1.200 | 0.270 | 0.362 | 0.225x | 0.302x |
| TPUv4 INT8 | 1.200 | 0.136 | 0.168 | 0.113x | 0.140x |
| TPUv5e FP16 | 0.859 | 0.193 | 0.259 | 0.224x | 0.301x |
| TPUv5e INT8 | 0.859 | 0.194 | 0.239 | 0.226x | 0.278x |
| TPUv5p FP16 | 2.765 | 0.450 | 0.603 | 0.163x | 0.218x |
| TPUv5p INT8 | 2.765 | 0.453 | 0.557 | 0.164x | 0.202x |
| TPUv6e FP16 | 1.600 | 0.503 | 0.856 | 0.315x | 0.535x |
| TPUv6e INT8 | 1.600 | 0.302 | 0.582 | 0.189x | 0.364x |
...
| TPUv3 INT8 | FlashAttention-CustomSA | 126.387 | fused_core | 126.087 | 99.76% |
...
| TPUv4 INT8 | FlashAttention-CustomSA | 104.416 | fused_core | 104.116 | 99.71% |
...
| TPUv5e INT8 | FlashAttention-CustomSA | 73.362 | fused_core | 73.062 | 99.59% |
...
| TPUv5p INT8 | FlashAttention-CustomSA | 31.612 | fused_core | 31.312 | 99.05% |
...
| TPUv6e INT8 | FlashAttention | 58.051 | softmax | 57.751 | 99.48% |
| TPUv6e INT8 | FlashAttention-CustomSA | 30.278 | fused_core | 29.978 | 99.01% |
...
- `TPUv3 INT8` at context 32768: `baseline` is dominated by `softmax` (192.574 ms), `flashattention` is dominated by `matmul_pair` (143.334 ms), and `customsa` is dominated by `fused_core` (126.087 ms).
...
- `TPUv5e INT8` at context 32768: `baseline` is dominated by `softmax` (233.367 ms), `flashattention` is dominated by `matmul_pair` (89.823 ms), and `customsa` is dominated by `fused_core` (73.062 ms).
...
- `TPUv5p INT8` at context 32768: `baseline` is dominated by `softmax` (56.340 ms), `flashattention` is dominated by `matmul_pair` (38.495 ms), and `customsa` is dominated by `fused_core` (31.312 ms).
...
- `TPUv4 INT8` at context 32768: `baseline` is dominated by `softmax` (161.404 ms), `flashattention` is dominated by `matmul_pair` (128.310 ms), and `customsa` is dominated by `fused_core` (104.116 ms).
...
- `TPUv6e INT8` at context 32768: `baseline` is dominated by `softmax` (148.533 ms), `flashattention` is dominated by `softmax` (57.751 ms), and `customsa` is dominated by `fused_core` (29.978 ms).
