...
- LLaMA2 W4 direct full-forward logits after the mask fix, qmodel loaded, no user mask: activation quant on gives mean diff , max diff  versus SDPA.
- Disabling activation quant for that diagnostic reduces the mean diff to , max diff ; dynamic activation quant amplifies small SDPA/manual arithmetic differences.
