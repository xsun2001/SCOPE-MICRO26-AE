...
Set `MODEL_ROOT` to a directory with all five model folders. Dataset downloads use Hugging Face caches. The packaged configuration and historical variant names are kept unchanged so the regenerated axis labels, titles, and plotting style match the submitted Figure 16. Consult `data/configs/` for the exact execution settings.
