...
class MyTrainer(transformers.Trainer):
    ...
    def get_ori_outputs(self, model, inputs):
        ...
        disabled_scna = []
        if getattr(args, "scna_teacher_exact", False):
            for module in unwrapped_model.modules():
                if hasattr(module, "scna_exp") and module.scna_exp is not None:
                    disabled_scna.append((module, module.scna_exp))
                    module.scna_exp = None

        set_temporary(unwrapped_model, False)
        set_quant_state(unwrapped_model, False, False, False)
        try:
            outputs = model(**inputs, output_hidden_states=True)
        finally:
            for module, scna_exp in disabled_scna:
                module.scna_exp = scna_exp

        set_temporary(unwrapped_model, True)
        set_quant_state(
            unwrapped_model, args.train_enable_wquant, True, args.fully_quant
        )
        return outputs
