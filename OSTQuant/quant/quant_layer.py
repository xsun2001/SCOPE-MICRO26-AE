...
def build_rotary_matrix(cos, sin):
    ...
    weight[:, : head_dim // 2] += sin_diag[:, head_dim // 2 :]
    return weight.reshape(bsz, seq_len, head_dim, head_dim)
...
class QuantAttention(nn.Module):
    def __init__(self, org_module: LlamaAttention, config: LlamaConfig, args):
        ...
        self.softmax = QuantSoftmax(
            ...
            -1,
            scna_dim=args.scna_dim if args.scna else None,
            scna_artifact_root=args.scna_artifact_root,
            scna_input_quant_bits=args.scna_input_quant_bits,
            scna_input_clip_min=args.scna_input_clip_min,
            scna_input_scale=args.scna_input_scale,
            scna_output_floor_log=args.scna_output_floor_log,
            intattention=args.intattention,
            intattention_exp_bits=args.intattention_exp_bits,
            intattention_zero_thr=args.intattention_zero_thr,
            intattention_output_bits=args.intattention_output_bits,
        )
    ...
    def forward(
        ...
        if self.use_sdpa:
            ...
            with torch.backends.cuda.sdp_kernel(True, True, True, False):
            ...
            if attention_mask is None and self.is_causal and q_len > 1:
                ...
                attention_mask = torch.triu(attention_mask, diagonal=1).view(1, 1, q_len, key_len)
