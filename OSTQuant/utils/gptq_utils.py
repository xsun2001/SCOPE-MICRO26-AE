...
def _stable_cholesky(H, diag, damp, label):
    H = (H + H.t()) * 0.5
    Hdiag = torch.diag(H).clone()
    finite_diag = torch.nan_to_num(Hdiag, nan=0.0, posinf=0.0, neginf=0.0)
    scale = finite_diag.abs().mean().clamp_min(torch.finfo(H.dtype).eps)
    base_damp = torch.nan_to_num(damp, nan=0.0, posinf=0.0, neginf=0.0).abs().clamp_min(scale * 1e-6)

    for attempt in range(8):
        attempt_damp = base_damp * (10 ** attempt)
        H_try = H.clone()
        H_try[diag, diag] = Hdiag + attempt_damp
        chol, info = torch.linalg.cholesky_ex(H_try)
        if int(info.max().item()) == 0:
            if attempt > 0:
                print(f"GPTQ {label}: Cholesky succeeded with damp={attempt_damp.item():.6e}", flush=True)
            return chol

    raise RuntimeError(f"GPTQ {label}: Cholesky failed after escalating damp from {base_damp.item():.6e}")
...
class GPTQ:
    ...
    def fasterquant(
        ...
        H = _stable_cholesky(H, diag, damp, "H")
        H = torch.cholesky_inverse(H)
        H = (H + H.t()) * 0.5
        H = _stable_cholesky(H, diag, torch.zeros((), device=self.dev, dtype=H.dtype), "Hinv").t()
        Hinv = H
